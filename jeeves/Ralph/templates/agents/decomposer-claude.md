...
- Creates task specifications (TASK.md, boilerplate files) and orchestrates Gate 1 architect review
...
1. **decomposer-architect** — for architecture, design, integration patterns, technology choices, Gate 1 per-task spec review, Gate 3 post-decomposition review
...
CHECK:
  ...
  - [ ] Gate 1 completed for task N before creating task N+1 (verify via todoread)
  - [ ] Recovery file updated after each task creation + Gate 1 review
...
| `PREPARING_RECOVERY` | Recovery file written | `CREATING_TASK` | Add per-task TODO items (ITT-01), then create task folder + TASK.md + Gate 1 | None |
| `CREATING_TASK` | TASK.md written, Gate 1 complete | `PROCESSING_TASK` | Incorporate findings, update recovery file | None |
| `PROCESSING_TASK` | Task processed | `CREATING_TASK` or `GENERATING_DEPS` | Next task or move to deps | None |
...
| `RE_ANCHORING` | PRD fully re-read, manifest cross-checked, session config verified | `CREATING_TASK` | Proceed to task creation | None |
...
- `CREATING_TASK`: Task folder created, TASK.md draft written, Gate 1 review invoked
- `PROCESSING_TASK`: Gate 1 feedback incorporated, recovery file updated
...
2.5. Call `todowrite` to add three items (all status: in_progress, priority: high):
   ...
   - `RE_ANCHORING [3/3]: Verify project type + package overrides + version numbers match before task creation`
...
| Bash | Directory creation, file operations, template copying |
...
todowrite([
  ...
  { content: "Create task specs + Gate 1 reviews (per-task items added when entering CREATING_TASK)", status: "pending", priority: "high" },
...
When entering the CREATING_TASK state, add a TODO item for EACH
planned task's creation:
...
1. TASK.md written and Gate 1 review completed (if required)
...
- **After each task creation + Gate 1 review**: Mark that task's creation item completed
...
Create tasks ONE AT A TIME. Do NOT create multiple task folders in parallel. Each task must be fully created, Gate 1 reviewed, and recovery file updated before starting the next.
...
**The decomposer-architect MUST review specs for implied requirement completeness** (see Gate 1 in SUB-AGENT REVIEW PROTOCOL).
...
Before starting, add a TODO item for every planned task (per Task Creation Tracking). Then for each task, create the specification directly.
...
```
FOR each task:
  1. Create folder: mkdir -p /proj/.ralph/tasks/{TASK_ID}/
  2. Read TASK.md template, fill with task-specific content (see Filling Instructions below)
  3. Use sequentialthinking (minimum 5 thoughts) for implied requirement cross-reference analysis, scoped to this task's designated PRD sections only (see PRD Scope Boundary below; see Implied Requirement Analysis section for exploration questions: data states, missing entities, partial failures, boundary conditions, precondition assumptions; for UI: gesture disambiguation, touch tolerance, keyboard conflicts, visual feedback)
  3b. Run Pre-Write Completeness Check (see below)
  4. Write TASK.md to task folder
  5. [HARD GATE] If not infrastructure/documentation: invoke decomposer-architect for Gate 1 spec review
  6. Apply architect feedback to TASK.md (use judgment — don't blindly add everything)
  7. Create boilerplate files:
     a. Read activity.md template from /opt/jeeves/Ralph/templates/task/activity.md.template
     b. Write activity.md to /proj/.ralph/tasks/{TASK_ID}/activity.md with task-specific title
     c. Read attempts.md template from /opt/jeeves/Ralph/templates/task/attempts.md.template
     d. Write attempts.md to /proj/.ralph/tasks/{TASK_ID}/attempts.md with task-specific title
  8. Update recovery file (decomposition-session.md): mark task completed, append learnings
  9. Mark task's TODO item as complete (ONLY AFTER step 8)
  10. Only THEN proceed to next task
```

**BATCHING IS A VIOLATION:** Do NOT create multiple task folders or write multiple TASK.md files in parallel. Each task must be fully created, reviewed, and recorded before starting the next.

#### Pre-Write Completeness Check (run before writing each TASK.md)

- [ ] Contracts Defined by prior tasks copied verbatim (not paraphrased) into this task's context where relevant
- [ ] Competing Interactions listed for UI tasks that share elements with other tasks (or "None" if no conflict)
- [ ] Learnings from prior tasks that affect this task included in constraints or implementation notes
...
- [ ] Package name overrides reflected (if manifest says "use X instead of Y", this task says X)

#### PRD Scope Boundary

For each task, identify ONLY the PRD sections designated as primary scope. Cross-reference analysis (Step 3 in the per-task loop) MUST be scoped to those sections. Requirements found in other PRD sections should be noted as cross-task impacts in the Processing Results step, NOT added to this task's specs.

#### TASK.md Filling Instructions (DEC-P1-SCOPE: WHAT/WHY, not HOW/WHERE)

1. Read the template from `/opt/jeeves/Ralph/templates/task/TASK.md.template`. If template not found, construct TASK.md using these section headers as fallback: Title, Behavioral Specifications, Acceptance Criteria, Constraints, Workflow Context, Version References, Validation Steps, Implementation Notes, E2E Test Scope, Documentation Criteria.
2. Fill the template section by section:
   - **Title**: Action-oriented title using agent-routing keywords (see DEC-P1-ROUTE)
   ...
   - **Constraints**: Include the standard ENV-P0 block PLUS any task-specific constraints
   - **Workflow Context**: Task type, review task reference, full suite regression check
   - **Version References**: Include validated version information from version manifest
   - **Validation Steps**: Non-interactive bash commands. Include ALL test runners from manifest (REQUIRED for implementation tasks).
   ...
   - **E2E Test Scope**: If Distributed E2E strategy, include E2E scenarios for this task's user flows (N/A for infrastructure/documentation tasks)
   - **Documentation Criteria**: At least one documentation acceptance criterion
3. **Scope boundary**: Specify WHAT/WHY, never HOW/WHERE. The implementing agent decides implementation approach.

#### Gate 1: Per-Task Spec Review

After drafting each TASK.md (for implementation tasks, NOT infrastructure/documentation), invoke decomposer-architect:

1. Include the standalone consultation preamble (see Sub-Assistant Invocation)
2. Include in the delegation:
   - Path to the TASK.md: `/proj/.ralph/tasks/{TASK_ID}/TASK.md`
   - PRD path
   - Cross-task context: contracts from prior tasks, competing interactions, learnings from prior task results — so the architect can evaluate integration completeness
   - Evaluation criteria:
   ...
   - **Adaptive response protocol instructions** (include these in the delegation):
...
3. **If architect invocation fails or returns no parseable response:**
   ...
   2. If the second attempt also fails: STOP. Do NOT skip Gate 1 or proceed.
      Inform the user that the architect review failed for this task and request guidance.
      Do NOT skip to the next task.

4. **Parse the architect's response:**
   - Extract VERDICT (APPROVE or REVISE) from the first line
   - Determine if findings are inline or file-based
   - If file-based: read the review file from `/tmp/decomposer-reviews/{TASK_ID}-gate1-review.md`, then delete it after processing

5. **If VERDICT is REVISE:** Apply specific fixes to TASK.md via the edit tool. Use judgment — do NOT blindly add everything the architect suggests. If a finding requires research you cannot do inline, note it for decomposer-researcher (see Processing Each Task's Results below).

6. **If VERDICT is APPROVE:** Proceed to boilerplate creation.
...
#### Processing Each Task's Results

After creating each task + completing Gate 1, capture and record:

1. **Key Learnings**: Findings from cross-reference analysis and Gate 1 for subsequent tasks
2. **Cross-Task Impacts**: Dependency changes, interactions with other tasks discovered during analysis
3. **Contracts Defined** (if any): Structured list of interfaces this task defines, formatted as: `name | signature/shape | expected file path`. Copy these verbatim into cross-task context for subsequent tasks that depend on these contracts.
4. **Gotchas & Decisions**: Edge cases, risks, judgment calls made during spec writing
5. **Findings Not Applied** (if any): Architect suggestions NOT added to TASK.md, with reasoning for each. Evaluate whether rejections are justified before proceeding.
6. **SCOPE_WARNING**: Set to `OVERSIZED` if the task scope appears too large for a single agent session, or `AMBIGUOUS` if the scope was unclear. If either is set, evaluate whether to split the task or clarify scope before proceeding to the next task.
7. If architect flagged `RESEARCH_NEEDED`: invoke decomposer-researcher to resolve, then re-draft TASK.md with findings (full standard flow: draft → Gate 1 → update recovery file)
8. Update recovery file with completed task status + all findings above
...
#### Initial Write (after Step 5, before first task creation)
...
Status values: `completed` (all gates passed) | `pending` (not yet started) | `in-progress` (treat as `pending` on recovery)
...
#### Iterative Update (after each task creation + Gate 1)

- Update task status: pending → completed
- Append key learnings
- Note cross-task impacts discovered
- **Version Manifest Sync**: If any finding corrects a version or package name, update the Version Manifest Summary immediately
- Update Scope Descriptions for remaining tasks affected by cross-task impacts
...
2. Has scope descriptions for remaining tasks ready to use
3. Has accumulated learnings to include in subsequent tasks
4. Does NOT need to re-read existing TASK.md files

#### Post-Compaction Re-Anchoring [MANDATORY]
Inlined at Post-Compaction Resume Detection (steps 3–4). States: `COMPACTION_RECOVERY` → `RE_ANCHORING` → `CREATING_TASK`.

**Task Status on Recovery:**
- Tasks marked `completed` (all gates passed): Keep as-is. Do NOT re-create.
- Tasks marked `in-progress` or `pending`: Treat as fresh TODOs. Any existing files for these tasks should be replaced. Execute the full standard flow: create folder → write TASK.md → Gate 1 review → update recovery file.
...
- Options to present:
  1. **Re-create affected tasks** — re-create TASK.md for each affected task (full standard flow: draft → Gate 1 review → update recovery file). Existing files replaced.
...
1b. Ensure all task specifications created with Gate 1 reviews completed
...
9. **DEC-P1-REVIEW**: Non-infra/non-docs tasks had Gate 1 (direct architect review), Gate 3 completed, Gate 2 done for applicable tasks, all feedback incorporated, SCOPE_WARNING evaluated
...
After drafting EACH TASK.md, invoke decomposer-architect to review the spec for completeness. This is NOT optional.

**"Per-task" means ONE architect invocation PER implementation task.** Sending all tasks in a single batch review is Gate 3, not Gate 1. If you have 12 implementation tasks, Gate 1 runs 12 times.

**Delegation details**: See Step 6, "Gate 1: Per-Task Spec Review" subsection for the full delegation protocol.
...
| 13 | DEC-P1-REVIEW | Gate 1 (architect review confirms), Gate 2 (researcher, if triggered), Gate 3 (architect post-decomp) |
...
|---------------|------------|
| **decomposer-architect** | Gate 1 per-task spec review, Gate 3 post-decomposition review, design decisions, integration patterns, tech stack, API contracts |
