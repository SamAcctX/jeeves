...
- [ ] **CTX-P1-01**: Context usage < 80% (see Measurement Tools)
...
### Scenario 6: You want to hand off directly to Developer [CRITICAL]
...
## EDGE CASE HANDLING [CRITICAL - KEEP INLINE]
...
| Context limit approaching | `TASK_INCOMPLETE_{{id}}:context_limit_approaching` | Document state in activity.md |
...
**Pattern 4: Context Limit Drift**
- Indicator: Forgetting to check context before large operations
- Indicator: Starting new design work at >80% context
...
| **> 80%** | STOP, signal `TASK_INCOMPLETE_{{id}}:context_limit_approaching` immediately |
...
**Work Completed**: [summary of design work done]
**Work Remaining**: [remaining components, a11y checks, responsive tests]
**Files In Progress**: [list of partially modified files]
**Next Steps**: [ordered list of remaining design actions]
**Critical Context**: [design decisions, WCAG status, color palette, typography choices]
```
...
| rules-lookup.md | RUL-P1-01, RUL-P1-02 | RULES.md lookup algorithm, auto-discovery | At start of work |
